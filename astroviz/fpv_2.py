...
TARGET_FRAME_MS      = 1000.0/max(1, DEFAULT_RENDER_HZ)
...
class FPVViewer(QMainWindow):
    def __init__(self, node: Node, config: dict | None):
        ...
        self.frame_combo = QComboBox(self.gl); self.frame_combo.setFixedWidth(220); self.frame_combo.raise_()
        self.frame_combo.addItems(["RAW (source)"])
        ...
        self.frames_timer= QTimer(self); self.frames_timer.timeout.connect(self._populate_frames); self.frames_timer.start(1200)
        ...
        self._last_msg = None  # (msg, t_receive)
    ...
    def resizeEvent(self, ev):
        ...
        self.frame_combo.move(x,y); x+= self.frame_combo.width()+m
    ...
    def _populate_frames(self):
        try: yaml_txt = self.tf_buffer.all_frames_as_yaml()
        except Exception: return
        frames = self._parse_frames_yaml(yaml_txt)
        items = ["RAW (source)"] + frames
        if [self.frame_combo.itemText(i) for i in range(self.frame_combo.count())] != items:
            cur = self.frame_combo.currentText()
            self.frame_combo.blockSignals(True)
            self.frame_combo.clear(); self.frame_combo.addItems(items)
            if cur in items: self.frame_combo.setCurrentText(cur)
    ...
    def _decode_worker(self):
        ...
        while not self._stop:
            ...
            if color_mode in ("rgb","rgba"):
                b4 = arr_struct["rgba" if color_mode=="rgba" else "rgb"].view(np.uint8)
                n4 = (b4.size // 4) * 4
                b4 = b4[:n4].reshape(-1, 4)
                if msg.is_bigendian:
                    r = b4[:,1]; g=b4[:,2]; b=b4[:,3]; a=b4[:,0] if color_mode=="rgba" else np.full(b4.shape[0],255,np.uint8)
                else:
                    b = b4[:,0]; g=b4[:,1]; r=b4[:,2]; a=b4[:,3] if color_mode=="rgba" else np.full(b4.shape[0],255,np.uint8)
                rgba = np.empty((b4.shape[0],4), dtype=np.float32)
                rgba[:,0] = r.astype(np.float32)/255.0
                rgba[:,1] = g.astype(np.float32)/255.0
                rgba[:,2] = b.astype(np.float32)/255.0
                rgba[:,3] = a.astype(np.float32)/255.0
            elif color_mode == "sep":
            ...
            self._last_decode_ms = (t1-t0)*1000.0

    def _apply_transform(self, xyz: np.ndarray, target_frame: str, source_frame: str, at_time: Time)->np.ndarray:
        if source_frame is None or target_frame=="RAW (source)" or target_frame==source_frame:
            return xyz
        try:
            tf=self.tf_buffer.lookup_transform(target_frame, source_frame, at_time, timeout=Duration(seconds=0.01))
        except Exception:
            try:
                tf=self.tf_buffer.lookup_transform(target_frame, source_frame, Time(), timeout=Duration(seconds=0.01))
            except Exception:
                return xyz
        T=tf_to_matrix(tf)
        R=T[:3,:3]; t=T[:3,3]
    ...
    def _refresh(self):
        ...
        target = self.frame_combo.currentText()
        xyz = self._apply_transform(xyz, target, src_frame, stamp)
        ...
        t1 = time.perf_counter()
        self._last_upload_ms = (t1 - t0)*1000.0
        total_ms = self._last_upload_ms

        if total_ms > 1.5*self._target_ms and self.max_points > DEFAULT_MAX_POINTS//4:
            self.max_points = max(DEFAULT_MAX_POINTS//4, int(self.max_points*0.8))
        elif total_ms < 0.7*self._target_ms and self.max_points < DEFAULT_MAX_POINTS:
            self.max_points = min(DEFAULT_MAX_POINTS, int(self.max_points*1.1))
...
def load_config(path: str | None)->dict | None:
    ...
    if yaml is None:
        print("[WARN] PyYAML is not available. Ignoring configuration.")
        return None
    ...
    try:
        ...
        print(f"[WARN] Failed to load YAML ({e})."); return None

def parse_args():
    ap = argparse.ArgumentParser(description="FPV LiDAR Viewer (ultra-fast) with YAML")
    ap.add_argument("--config", type=str, default=None, help="YAML path")
