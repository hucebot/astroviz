...
class GPSMapWindow(QMainWindow):
    ...
    def toggle_waypoint_mode(self):
        self.node.create_subscription(NavSatFix, '/ublox_7/sensor/gps', self.on_gps, 10)
        self.node.create_subscription(Int32, '/ublox_7/sensor/satellites', self.on_sats, 10)
    ...
    def on_sats(self, msg: Int32):
        ...
        self.sat_label.setText(
            f": <b><span style='color:{color}'>{sats}</span></b>"
        )
              ...
              window.wp_list.forEach(coord => {
                ...
                if (d <= 5) {
                  const circle = L.circle(coord, {
                    ...
                    radius: 2 // small radius in meters, visual highlight
                  }).addTo(map);
            ...
            window.updatePosition = async function() {
              try {
                ...
                if (!firstFix) {
                  ...
                  wp_poly = L.polyline([], {color:'blue', weight:3, opacity:0.4}).addTo(map);
                  return;
                }
